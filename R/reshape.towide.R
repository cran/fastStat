reshape_towide<-function (data, id, j, prefix, suffix)
{
    ...
    if (length(id.left.names) == 1) {
        ...
        idsplitid.j = make.unique(as.character(trans.data[, j]),
                                  sep = "idsplitid")
        idsplitid.j = reshape2::colsplit(idsplitid.j, "idsplitid",
                                         c("j", "id"))
        do.id = ifelse(is.na(idsplitid.j[, 2]), 0, idsplitid.j[,
                                                               2]) + 1
        id.left.names = "nrow"
    }
    data2 = cbind(do.id, trans.data)
    wide.1 = stats::reshape(data = data2, idvar = "do.id", timevar = j,
                            direction = "wide", sep = "")
    wide.2 = wide.1[, -1]
    if (!missing(suffix)) {
        ...
        for (i in 1:length(suffix)) {
            ...
            suffix.loc = (1:length(suffix.char))[suffix[i] ==
                                                     suffix.char]
            name.suffix = do::Replace0(colnames(wide.2)[suffix.loc],
                                       suffix[i])
            colnames(wide.2)[suffix.loc] = paste0(name.suffix,
                                                  suffix[i])
        }
    }
